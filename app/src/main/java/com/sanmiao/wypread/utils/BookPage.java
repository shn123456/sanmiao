...
public class BookPage extends View {
    ...
    public BookPage(Context context,int w,int h) {
        super(context);
        mPath0 = new Path();
        mPath1 = new Path();
        mScreenWidth = w;
    ...
    public void startAnimation2(int w,int h,int delayMillis){
        int dx, dy;
        if (mCornerX > 0) {
            dx = -(int) (w + 950.0);
        } else {
        ...
        if (mCornerY > 0) {
            ...
            dy = (int) (1 - 1052.0); // 防止mTouch.y最终变为0
        }
        mScroller.startScroll(950, 1052, dx, dy, delayMillis);
    }
